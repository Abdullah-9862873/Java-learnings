...
public class LL {
    ...
    public void display() {
        Node temp = head; // Here temp will start pointing to the same node where head is pointing in the
                          // heap memory
        while (temp != null) {
    ...
    public LL mergeTwoListsMain(LL list1, LL list2) {
        LL ans = new LL();
        ...
        while (current1 != null && current2 != null) {
            if (current1.value < current2.value) {
                ans.insertAtEnd(current1.value);
                current1 = current1.next;
            } else {
                ans.insertAtEnd(current2.value);
